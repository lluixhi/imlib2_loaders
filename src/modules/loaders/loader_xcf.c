	...
	case PROP_COLORMAP:
	  {
		...
		image->cmap = (DATA8*) malloc (sizeof(DATA8) * image->num_cols * 3);
...
static char
xcf_load_level (Tile** tiles_p, int hierarchy_width, int hierarchy_height, int bpp, int* num_rows, int* num_cols)
{
  DATA32 saved_pos;
...
{
  image->cp += xcf_read_int8 (image->fp, tile->data, tile->ewidth * tile->eheight * tile->bpp);
  return 1;
}
...
flatten_image(void)
{
  ...
  image->data = (DATA8*) calloc(image->width * image->height, sizeof(DATA32));
  layer_index = 0;
